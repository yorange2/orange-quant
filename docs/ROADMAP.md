...
### 追加：成本敏感度（2026-08-20，上线后补测）

上线前一直欠着的一项。`run_hour_designs.py` 新增 `--cost-rate` 维度，
2 设计 × 2 宽度 × 3 费率 × 24 锚点 = 288 次回测：

Sharpe（24 锚点均值），**池化×滚动24h（部署中）**：

| topk | 年换手 | 0.1% | 0.2% | 0.3% | Δ(0.1→0.2) |
|---|---|---|---|---|---|
| 10 | 25.2 | 2.736 | 2.490 | 2.258 | −0.246 |
| **20（现用）** | 16.1 | 2.237 | 2.083 | 1.936 | −0.154 |

池化单模型对照：topk=10 换手 25.4，Sharpe 1.530/1.348/1.176；topk=20 换手 14.7，
Sharpe 1.234/1.131/1.031。

- **成本不构成威胁**：费率翻倍 Sharpe 只掉 0.15~0.25，且 0.1→0.2 与 0.2→0.3 的
  掉幅几乎相同——**线性衰减，无成本悬崖**。0.3% 的极端费率下 topk=20 仍有 1.936。
- **年成本绝对量级**：topk=20 在 0.1% 下 1.6%/年、0.2% 下 3.2%/年。
- **topk=10 的优势穿透成本**：0.1% 领先 0.499、0.2% 领先 0.407、0.3% 领先 0.322。
  换手高 56%（每年多花约 2.5%）吃掉一部分但吃不掉全部。
- **但仍不改 topk**：topk=10 跨锚点 σ 0.995（topk=20 是 0.821）——成本测试证明了
  "优势不是被成本吃掉的假象"，没有证明"优势不是 179 天的运气"。且刚改了实盘死区，
  一次只动一个变量。

复现：`run_hour_designs.py --designs pooled pooled-ens-causal --topk 10 20
--cost-rate 0.001 0.002 0.003 --out cost_sweep`。

### 实盘参数：死区 `min_notional_safety` 20 → 6（2026-08-20）

滚动24h 上线后第二轮暴露的问题。`diff_orders` 跳过 `|目标−实际| < min_notional_safety`
的调整，该值原为 20：

- 账户权益 ~$1,260、topk=20 → 单仓 $44，**死区相当于单仓的 45%**；
- 实测**20/20 个仓位全部落在死区内**，若信号不变则一笔调整都不会发生；
- 后果已显现：部署比例 0.7188 vs 目标 0.7000，ETH 漂到 $61.58（超目标 40%）
  却因 delta $17.45 < 20 无法削减——**偏离随行情单向累积，系统无力纠正**。

这不是 bug 而是参数与账户规模不匹配：20 是为 $100k 账户设的（单仓 $3,500，占 0.6%）。
改为 6（Binance `MIN_NOTIONAL`=5，留缓冲），死区降到单仓的 14%。

⚠️ **`trading.min_trade` 是死配置**——`lgb/runner.py` 读取但代码中无任何使用点，
...
（只改配置不换 `state_file` 则不会补跑，实测重启后 `last acted 2026-08-20`，正常等待。）

#### 追加：topk 20 → 10（同日，**修订「成本敏感度」节的"仍不改 topk"**）

上节因"topk=10 跨锚点 σ 更大 + 一次只动一个变量"暂缓，现随 D 路线一并改。
D 路线下 24 锚点均值（cost=0.001）：

| topk | Sharpe | 跨锚点σ | 最差锚点 | IR | NAV | 年换手 | 最大回撤 |
|---|---|---|---|---|---|---|---|
| 5 | +3.507 | 1.378 | +1.713 | +2.174 | 2.484 | 43.6 | −0.208 |
| **10（现用）** | **+2.592** | 0.780 | +1.154 | +2.096 | 1.866 | 26.2 | −0.173 |
| 20（原） | +2.319 | 0.578 | +1.592 | +2.068 | 1.783 | 16.7 | −0.186 |
| 30 | +1.714 | 0.470 | +0.783 | +1.760 | 1.609 | 11.2 | −0.227 |

- **相对切换前是反转的**：原模型（B）下 topk=10 只有 1.358，**劣于** topk=20 的 1.531；
  D 下变为 2.592 > 2.319。与"污染集中在排序顶端、D 把顶端清干净了"一致——顶端可信
  之后，越集中越有利。这也是上节暂缓理由失效的原因。
- **但配对检验无法区分**：分块自助（超额收益）10−20 = **+0.068，95%CI
  [−0.662, +0.707]，P(≤0)=0.42**；子窗口 2/3 段为正但第三段 −0.901。
- **代价明确**：年换手 16.7 → 26.2（+57%），0.1% 费率下年成本 1.7% → 2.6%；优势随
  费率快速衰减（0.1% +0.27 / 0.2% +0.19 / **0.4% 仅 +0.03 ≈ 打平**），而实盘市价单
  在小币上的滑点会把有效费率推到 0.1% 以上。分散度也降了（σ 0.780 vs 0.578，
  最差锚点 +1.154 vs +1.592）。
- ⚠️ **与 D 路线切换同日生效，一次动了两个变量**，后续若表现不及预期将无法归因到
  其中某一个。这是明知的取舍，不是疏漏。
- 连带：单仓 ~$44 → ~$89，死区占单仓比例 14% → 7%（更宽松，无需另调）；
  `max_position_pct=0.25` 对 0.7/10=0.07 无约束；`min_universe=30` 余量更大。

### 实盘滑点实测：真实成本 vs `cost_rate`（2026-08-21，`scripts/measure_slippage.py`）

回测按 `cost_rate=0.1%` 单边计费，且假定成交价**精确等于**决策那根 bar 的 close。实盘
两条都不成立：成交发生在 bar 收盘后 2~7 分钟，且吃的是盘口另一侧。新脚本从 Binance
`fetchMyTrades` 拉真实成交（**不依赖每轮被覆盖的 state 文件**，样本随轮次自然累积），
对比 `latest_closed_bar(成交时刻)` 的 close——与 `Rolling24hScorer` 判定"最新已收盘 bar"
的规则完全一致，所以比的是模型当时真正看到的那个价。

71 单 / 152 个 fill / 5 轮 / 名义额 $4,361：

| | bp |
|---|---|
| 手续费（Binance taker 实收） | 9.79 |
| 买入滑点 | +10.59 |
| 卖出滑点 | −7.54 |
| **半价差 h =（买+卖）/2** | **+1.53** |
| **市场漂移 d =（买−卖）/2** | **+9.06** |
| **真实单边成本 = 手续费 + 2h** | **12.84**（回测 10.0，**1.28×**） |

- **滑点必须拆成两块**：买卖滑点**符号相反且量级相近** → 主体是 bar 收盘到成交这几分钟
  的市场漂移（随机，长期趋 0），不是价差成本。设成交时刻 `mid = close×(1+d)`、半价差 `h`，
  则买入滑点 `= d+h`、卖出滑点 `= −d+h`，两式解出 h 与 d。不拆开的话，一段单边行情会被
  整个误读成滑点（今早 BTC 成交价高出那根 close 0.44%，全属漂移）。
- **年成本**：换手 26.2 下，回测 2.62% → 实际 3.37%。
- **修正上一节的猜测**：topk 节写的"实盘滑点会把有效费率推到 0.1% 以上"方向对，但幅度远
  小于当时的暗示——实际 0.128%。按 cost sweep 插值，topk=10 对 topk=20 的优势在此费率下
  仍有 ≈ +0.24（0.1% 时 +0.27、0.2% +0.19、0.4% 才 +0.03）。**成本不构成对 topk=10 的
  威胁**；但这不改变配对自助"两者在时间维上无法区分"的结论，只是排除了"被成本吃掉"这一
  条特定担心。
- **Roll 价差估计不能当成本用**：先前据 Roll 估出 BANK 1.094%、EPIC 1.375%，据此担心这
  几个仓位的成本被系统性低估。实测 **BANK 滑点 +0.00 bp、EPIC −40.68 bp（有利）**。Roll
  测的是收盘价序列的统计抖动（含低流动性时段的报价跳动），而我们单笔仅 $45~90，只吃盘口
  最顶一层。**Roll 是市场质量指标，不是小额市价单的成本估计。**
- **单币那一列不可用**：每币仅 2~6 单，KAITO +86bp ~ ENA −47bp 全是漂移噪声。
- 顺带：**EURI 手续费 0.00 bp**（稳定币对免手续费）。

⚠️ **踩过的坑**：Binance 买入时默认从**买到的币**里扣手续费（`fee.currency` 是币而非
USDT），直接算 `fee.cost / trade.cost` 会把币的数量当成金额，得出 BICO 296bp、BANK
278bp 这种离谱费率，而且**不会报错**。必须先按成交价折成计价货币。修正后所有非稳定币
手续费精确落在 10.00 bp——这本身就是最好的校验。

样本限制：5 轮中只有 2 轮是当前配置，`h` 依赖买卖对称性假设，总名义额仅 $4.3k。每天重跑
一次即可累积。「年成本 3.37%」里的换手 26.2 仍是回测**预测值**，攒够常态轮次后应替换为
实测换手再算。

用法：`../.venv/bin/python scripts/measure_slippage.py --days 30 --by-round`
